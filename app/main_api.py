...
async def get_ave_good_topN(top_n: int = Query(default=10, alias='top_n')) -> List[Dict[str, float]]:
...
####################################################################
# 3️⃣平均フォロワー数の多いインフルエンサー上位N件を取得
####################################################################
@app.get("/influencers/top-by-followers")
async def get_ave_flwr_topN(top_n: int = Query(default=10, alias='top_n')):
